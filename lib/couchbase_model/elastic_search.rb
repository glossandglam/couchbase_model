  ...
  module ElasticSearch
    ...
    module ClassMethods
      ...
      def search_wildcard(attribute, value, options = {})
        es_attr = CouchbaseModel::ElasticSearch.elasticsearch_attributes(self)
        value = CouchbaseModel::ElasticSearch.format_fields(value, es_attr[attribute])
        query = {wildcard: { attribute => { value: (options[:both_sides] ? "*" : "")+"#{value}*"}}}
        if options[:where].is_a? Hash
          query = { filtered: {query: query, filter: {bool: { must: CouchbaseModel::ElasticSearch.elasticsearch_filters(options[:where], es_attr)}}}}
        end
      ...
      def search_all(options = {})
        ...
        if options[:where].is_a?(Hash)
          if query.empty?
            query = { filtered: { filter: { bool: { must: CouchbaseModel::ElasticSearch.elasticsearch_filters(options[:where], es_attr) }}}}
          else
            query = { filtered: { query: query, filter: { bool: { must: CouchbaseModel::ElasticSearch.elasticsearch_filters(options[:where], es_attr) }}}}
          end
        end
      ...
      def elasticsearch_create_bulk_update(list, do_change = false)
        ...
        list.each do |model|
          ...
          out << CouchbaseModel::ElasticSearch.format_update(model, update: true, change: do_change)
    ...
    def self.format_fields(value, options)
      ...
      when :hash
        return CouchbaseModel::ElasticSearch.hasher(value, options[:structure]) || nil
      end
    
      case options[:format]
      when :date_time_no_millis
        d = CouchbaseModel::ElasticSearch.ensure_date value
      ...
      options.each do |attribute, opts|
        ...
        if data[attribute].is_a?(Array)
          ...
          data[attribute].each do |d|
            out[attribute] << CouchbaseModel::ElasticSearch.format_fields(d, opts)
          end
        else
          out[attribute] = CouchbaseModel::ElasticSearch.format_fields data[attribute], opts
        end
      end
      out
    end
        ...
        when :and
          ...
          if v.is_a? Hash
              ...
              when :range
                ...
                vv.each do |ty, val|
                  if [:gt, :lt, :gte, :lte].include? ty.to_sym
                    rg[ty] = CouchbaseModel::ElasticSearch.format_fields(val, opts[n.to_sym])
                  else
                    rg[ty] = val
                  end
                end
            ...
            if v.none?{|vv| vv.is_a?(Hash)}
              filters << { terms: { n => v.map{|vv| CouchbaseModel::ElasticSearch.format_fields(vv, opts[n.to_sym])}}}
            else
              filters << self.elasticsearch_filters({or: v.map{|vv| {n => vv}}}, opts[n.to_sym])
            end
          else
            filters << { term: { n => CouchbaseModel::ElasticSearch.format_fields(v, opts[n.to_sym])}}
          end
        end
      end
      ...
      while tries < MAX_CONFLICT_TRIES
        begin
          self.class.elasticsearch_client.index index: self.class.elastic_search[:index], type: self.class.elastic_search[:type], id: self.id,
            body: CouchbaseModel::ElasticSearch.format_update(self)
          break
        rescue Exception => e
          tries += 1
        end
    ...
    def self.format_update(model, options = {})
      es_attr = CouchbaseModel::ElasticSearch.elasticsearch_attributes(model.class) if options[:es_attr].nil?
